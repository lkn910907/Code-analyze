...
int nf10priv_xmit(struct nf10_card *card, struct sk_buff *skb, int port)
{
            ...
            (atomic64_read(&card->mem_tx_pkt.cnt) + cl_size <= card->mem_tx_pkt.cl_size))
    {
        ...
        /*函数pci_map_single映射单个用于传送的缓冲区，返回值是可以传递给设备的总线地址
        在流式 DMA 映射的操作中，缓冲区传送方向应匹配于映射时给定的方向值,PCI_DMA_TODEVICE 数据被发送到设备	*/
    ...
    // write to the card
    //如果在程序某处插入了mb()/rmb()/wmb()宏，则宏之前的程序保证比宏之后的程序先执行，从而实现串行化
    mb();
...
int nf10priv_send_rx_dsc(struct nf10_card *card)
{
	...
	这个函数改变data和tail指针，而data和tail指针分别指向负载的开头和结尾。这个函数通常在分配缓冲区之后就调用，*/
